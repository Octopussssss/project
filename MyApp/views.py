...
def insert_deal_method(request):
    ...
    print(type(d))#<class 'App.models.Patient'>没first()时是：<class 'django.db.models.query.QuerySet'>
    print(d)#Patient object  ,没first()时是：<QuerySet [<Patient: Patient object>]>
    print(type(d.id))#<class 'int'>    #外键不能赋予特定的值，而应该赋予具体的对象，如Patient object
...
def return5(request):
    ...
    if request.method == "GET":
        ...
        context={
            ...
            "work_dept":dict1["work_dept"],
            "depts": dict1["depts"],
            "doctor_results": dict1["doctor_results"],
            "work_time_results":dict1["work_time_results"]
        }
        return render(request, 'Doctor/doctor_work_time.html',context=context)
def return6(request):
    if request.method == 'GET':
